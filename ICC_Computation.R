...
Icc_Period_Get <- function(DateAnnounced,DateEffective){
        ...
        IccPraeDat$mon <- IccPraeDat$mon - (0:4)
        IccPostDat$mon <- IccPostDat$mon + (0:4)
        IccPraeDat <- substring(IccPraeDat,1,7)
        IccPostDat <- substring(IccPostDat,1,7)    
        MinObs <-"hier"
        Size  <- "hier"
...
Icc_Get<- function(DataPropList, TableStored, access_data = AccessData, Sample=FALSE){
        ...
        ## Zwischenspeicherung der Zeitfenster Daten
        DatPrae <- sort(PeriodData$DatPrae)                 ###XXX
        DatPost <- sort(PeriodData$DatPost)                 ###XXX
        ##MinObs  <- PeriodData$MinObs
        ##Size    <- PeriodData$Size
        ...
        ## data.frame zur Speicherung der ICCs
        ICC <- rep(NA,length(DatPrae))                      ###XXX
        ##ICC <- rep(NA,2)                                  ###XXX
        
        ## Am Ende soll ein Dataframe mit ICC Zeitreihen für das Target vor, den
        ## Acquiror vor und nach M&A im ICC data.frame gespeichert werden.
        ## Der data.frame "ICC" ist die leere Vorlage.
        ##ICC <- data.frame("TaIcc" = ICC, "AcIccPrae" = ICC,"AcIccPost" = ICC)   ###XXX
            ICC <- list("TaIcc"=data.frame("TaIccDate" = DatPrae,"TaIcc" = ICC, stringsAsFactors =F),              ###XXX
               "AcIccPrae"=data.frame("AcIccPraeDate" = DatPrae, "AcIccPrae" = ICC, stringsAsFactors =F),          ###XXX
               "AcIccPost"=data.frame("AcIccPostDate" = DatPost, "AcIccPost" = ICC, stringsAsFactors =F))          ###XXX
            ...
            ##tempDat <- is.element(tempTA[[DatCol]],DatPrae)
            ##tempDat <- tempTA[[DatCol]]==DatPrae                                  ###XXX   
            tempDat <- is.element(tempTA[[DatCol]],DatPrae)                         ###XXX
            ## Suche nach Einträgen zum prae Zeitfenster im Target ICC Datensatz
            IccExist <- is.element(DatPrae,tempTA[[DatCol]][tempDat])               ###XXX
            ##IccExist <- is.element(DatPrae,tempTA[[DatCol]][tempDat])       
            ## Um zu verhindern, dass fehlende Werte (nicht NA, sondern gar 
            ## kein Eintrag zu dem Datum) im Untersuchungszeitraum nicht
            ## verschwinden, müssen auch deren Positionen ermittelt werden und 
            ## beim Eintrag in die ICC Tabelle berücksichtigt werden        
           ## if(length(tempDat)==0|sum(tempDat)==0){ICC[1:2,"TaIcc"] <-NA}         ###XXX
           ## else{                                                                 ###XXX
           ##     ICC[1,"TaIcc"] <- tempTA[tempDat, DatCol,with=F]                  ###XXX
           ##     ICC[2,"TaIcc"] <- tempTA[tempDat, IccCol,with=F]                  ###XXX
           ##     }                                                                 ###XXX
            ICC[["TaIcc"]][IccExist,] <- tempTA[tempDat, c(DatCol,IccCol),with=F]   ###XXX    
            ##ICC[IccExist,"TaIcc"] <- tempTA[tempDat, IccCol,with=F]         
            ## Zuweisung der ICCs zum leeren data.frame "ICC"
            ...
            ##tempDat <- is.element(tempAC[[DatCol]],DatPrae)                 
            ##IccExist <- is.element(DatPrae,tempAC[[DatCol]][tempDat])
            ##tempDat <- tempAC[[DatCol]]==DatPrae                                      ###XXX   
            tempDat <- is.element(tempAC[[DatCol]],DatPrae)                             ###XXX
            ##ICC[IccExist,"AcIccPrae"] <- tempAC[tempDat, IccCol,with=F]        
            IccExist <- is.element(DatPrae,tempAC[[DatCol]][tempDat])                   ###XXX
            
            ##if(length(tempDat)==0|sum(tempDat)==0){ICC[1:2,"AcIccPrae"] <-NA}         ###XXX
            ##else{                                                                     ###XXX
            ##    ICC[1,"AcIccPrae"] <- tempAC[tempDat, DatCol,with=F]                  ###XXX
            ##    ICC[2,"AcIccPrae"] <- tempAC[tempDat, IccCol,with=F]                  ###XXX
            ##    }                                                                     ###XXX
            ICC[["AcIccPrae"]][IccExist,] <- tempAC[tempDat, c(DatCol,IccCol),with=F]   ###XXX     
            ##tempDat <- is.element(tempAC[[DatCol]],DatPost)     
            ##IccExist <- is.element(DatPost,tempAC[[DatCol]][tempDat])  
            tempDat <- is.element(tempAC[[DatCol]],DatPost)                             ###XXX
            IccExist <- is.element(DatPost,tempAC[[DatCol]][tempDat])                   ###XXX
            ##ICC[IccExist,"AcIccPost"] <- tempAC[tempDat, IccCol,with=F]
            ##if(length(tempDat)==0|sum(tempDat)==0){ICC[1:2,"AcIccPost"] <-NA}         ###XXX
            ##else{                                                                     ###XXX
            ##    ICC[1,"AcIccPost"] <- tempAC[tempDat, DatCol,with=F]                  ###XXX
            ##   ICC[2,"AcIccPost"] <- tempAC[tempDat, IccCol,with=F]                   ###XXX
            ##    }                                                                     ###XXX
            ICC[["AcIccPost"]][IccExist,] <- tempAC[tempDat, c(DatCol,IccCol),with=F]
...
Company_Get <- function(DataPropList, TableStored, access_data = AccessData){
	## Im Prinzip zu ICC_Get() analoge Bestimmung von Marktwerten
		...
		MV <- rep(NA,length(DatPrae))                 ###XXX
        ##Mv <- rep(NA,2)                               ###XXX
		##Mv <- data.frame("TaMv" = Mv, "AcMvPrae" = Mv,"AcMvPost" = Mv) ###XXX
        MV <- list("TaMv"=data.frame("TaMvDate" = DatPrae,"TaMv" = MV, stringsAsFactors =F),                      ###XXX
               "AcMvPrae"=data.frame("AcMvPraeDate" = DatPrae, "AcMvPrae" = MV, stringsAsFactors =F),              ###XXX
               "AcMvPost"=data.frame("AcMvPostDate" = DatPost, "AcMvPost" = MV, stringsAsFactors =F))              ###XXX
        
		temp <- CompanyTablePrae[[DscdCol]] == SdcData$TargetDscd                                                                                                 
        ##Mv$TaMv <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])    
        ##Mv[1,"TaMv"] <- DatPrae	                                                                                ###XXX
    
        ##Mv[2,"TaMv"] <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])                                       ###XXX	
        MV$TaMv[,2] <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])    
		## hier direkte Auswahl der Daten über Spaltennamen ohne is.element oder
        ## Überprüfung auf nicht verzeichnete Daten

		temp <- CompanyTablePrae[[DscdCol]] == SdcData$AcquirorDscd             
		##Mv$AcMvPrae <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])	  
        ##Mv[1,"AcMvPrae"] <- DatPrae                                                                               ###XXX
        ##Mv[2,"AcMvPrae"] <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])                                   ###XXX  
        MV$AcMvPrae[,2] <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])  
    
        ##Mv$AcMvPost <- as.numeric(CompanyTablePost[,DatPost,with=F][temp])           
		##Mv[1,"AcMvPost"] <- DatPost                                                                               ###XXX
        ##Mv[2,"AcMvPost"] <- as.numeric(CompanyTablePost[,DatPost,with=F][temp])                                   ###XXX
        MV$AcMvPost[,2] <- as.numeric(CompanyTablePrae[,DatPrae,with=F][temp])  
    ...
    ## Partner ähneln bzw. unterscheiden).
        ...
        AccessData <<- Data_Retrieve ()
            ...
            for (i_row in 1:laenge){ ## i_row für jede Zeile des SDC Datensatzes
                ...
                ## Zuweisung der Werte zu der M&A Tabelle    
            	##    maa_table[i_row,] <- c(as.character(AccessData$Get()$SDC$DatEffective),as.character(AccessData$Get()$SDC$DatAnnounced),     ###XXX
                ##    AccessData$Get()$SDC$AcquirorDscd, AccessData$Get()$SDC$TargetDscd,                                                         ###XXX
                ##    AccessData$Get()$SDC$ShareAc, AccessData$Get()$SDC$SicAc , AccessData$Get()$SDC$SicTa , NA,                                 ###XXX           
                ##    AccessData$Get()$ICC$TaIcc, AccessData$Get()$ICC$AcIccPrae, AccessData$Get()$ICC$AcIccPost,                                 ###XXX
                ##    AccessData$Get()$COMPANY$TaMv, AccessData$Get()$COMPANY$AcMvPrae, AccessData$Get()$COMPANY$AcMvPost)                        ###XXX
                
                    maa_table[i_row,] <- c(as.character(AccessData$Get()$SDC$DatEffective),as.character(AccessData$Get()$SDC$DatAnnounced), 
                    AccessData$Get()$SDC$AcquirorDscd, AccessData$Get()$SDC$TargetDscd, 
                    AccessData$Get()$SDC$ShareAc, AccessData$Get()$SDC$SicAc , AccessData$Get()$SDC$SicTa , NA,
                    AccessData$Get()$ICC$TaIcc[1,],AccessData$Get()$ICC$TaIcc[2,],AccessData$Get()$ICC$TaIcc[3,],AccessData$Get()$ICC$TaIcc[4,],AccessData$Get()$ICC$TaIcc[5,] ,
                    AccessData$Get()$ICC$AcIccPrae[1,],AccessData$Get()$ICC$AcIccPrae[2,],AccessData$Get()$ICC$AcIccPrae[3,],AccessData$Get()$ICC$AcIccPrae[4,],AccessData$Get()$ICC$AcIccPrae[5,], 
                    AccessData$Get()$ICC$AcIccPost[1,],AccessData$Get()$ICC$AcIccPost[2,],AccessData$Get()$ICC$AcIccPost[3,],AccessData$Get()$ICC$AcIccPost[4,],AccessData$Get()$ICC$AcIccPost[5,],
                    AccessData$Get()$COMPANY$TaMv[1,], AccessData$Get()$COMPANY$TaMv[2,],AccessData$Get()$COMPANY$TaMv[3,],AccessData$Get()$COMPANY$TaMv[4,],AccessData$Get()$COMPANY$TaMv[5,],
                    AccessData$Get()$COMPANY$AcMvPrae[1,], AccessData$Get()$COMPANY$AcMvPrae[2,],AccessData$Get()$COMPANY$AcMvPrae[3,],AccessData$Get()$COMPANY$AcMvPrae[4,],AccessData$Get()$COMPANY$AcMvPrae[5,],
                    AccessData$Get()$COMPANY$AcMvPost[1,],AccessData$Get()$COMPANY$AcMvPost[2,],AccessData$Get()$COMPANY$AcMvPost[3,],AccessData$Get()$COMPANY$AcMvPost[4,],AccessData$Get()$COMPANY$AcMvPost[5,])    
            }            
     ##DatEffective = DatEffective, DatAnnounced = DatAnnounced 
        ...
        ##IccColNames <- c(paste("TaIcc",-(nobs:1),sep="_"),paste("AcIccPrae",-(nobs:1),sep="_"),
        ##                paste("AcIccPost",(1:nobs),sep="_+"))
        ##IccColNames <- c("IccTaDate","TaIcc","IccAcPraeDate","AcIccPrae","IccAcPostDate","AcIccPost")                   ###XXX      
        
        TaNames <- c("IccTaDate","TaIcc")                                                                               ###XXX
        TaNames <- paste(TaNames,rep(5:1,each=2),sep="_-")                                                            ###XXX
        
        AcPraeNames <- c("IccAcPraeDate","AcIccPrae")                                                                       ###XXX
        AcPraeNames <- paste(AcPraeNames,rep(5:1,each=2),sep="_-")                                                            ###XXX
        
        AcPostNames <- c("IccAcPostDate","AcIccPost")                                                                       ###XXX    
        AcPostNames <- paste(AcPostNames,rep(1:5,each=2),sep="_+")                                                            ###XXX
        
        IccColNames <- c(TaNames, AcPraeNames, AcPostNames)                                                                  ###XXX
        
        ##MvColNames  <- c(paste("TaMv",-(nobs:1),sep="_"),paste("AcMvPrae",-(nobs:1),sep="_"),
        ##                paste("AcMvPost",(1:nobs),sep="_"))
        ##MvColNames  <- c("MvTaDate","TaMv","MvAcPraeDate","AcMvPrae","MvAcPostDate","AcMvPost")                           ###XXX
        
        TaNames <- c("MvTaDate","TaMv")                                                                               ###XXX
        TaNames <- paste(TaNames,rep(5:1,each=2),sep="_-")                                                            ###XXX
        
        AcPraeNames <- c("MvAcPraeDate","AcMvPrae")                                                                       ###XXX
        AcPraeNames <- paste(AcPraeNames,rep(5:1,each=2),sep="_-")                                                            ###XXX
        
        AcPostNames <- c("MvAcPostDate","AcMvPost")                                                                       ###XXX    
        AcPostNames <- paste(AcPostNames,rep(1:5,each=2),sep="_+")                                                            ###XXX
        
        MvColNames <- c(TaNames, AcPraeNames, AcPostNames)                                                                   ###XXX
        ...
        ## Änderung der Datentypen der Tabelle auf numeric
        num_col<-grep("Prae_|Post_|Mv_|ICC_|Perc_Shares",colnames(  maa_table))
        maa_table[,num_col]<-apply(maa_table[,num_col],2,as.numeric)
...
Calc_Weighted_Icc_prae <- function(maa_table = MaATable,acquiror_icc_prae = "AcIccPrae",target_icc_prae = "TaIcc",
            ...
            ## Erstellung vom leeren Datensatzes als Vorlage
            weighted_icc <- data.frame("WeightedIccPrae_-1"= rep(NA,n_row))
...
##5.4	Calc_Icc_Diff(): Berechnet aus gewichteten ICCs und korrespondierenden post M&A ICC des 
#####   Acquirors die ICC Differenz, die durchc den M&A entstanden ist.            
Calc_Icc_Diff <- function(maa_table = MaATable, weighted_icc_prae ="WeightedIccPrae", acquiror_icc_post ="AcIccPost"){
            ## Bestimmung, wieviele Berechnungen im Intervall vor M&A durchgeführt wurden
            n_icc_calc <- length(grep(weighted_icc_prae,colnames(maa_table)))
            n_row <- nrow(maa_table)
            ...
            for(i in 1:n_icc_calc){
                   ...
                   acquiror_icc_col <- maa_table[paste(acquiror_icc_post,i,sep="_+")]
...
Calc_Adj_Icc_Diff <- function(maa_table = MaATable, icc_diff ="IccDifference", acquiror_mv_prae="AcMvPrae",
                                target_mv_prae = "TaMv",share_acquired = "Perc_Shares_Acquired"){
                n_icc_calc <- length(grep(icc_diff,colnames(maa_table)))
                n_row <- nrow(maa_table)
              
                ## Erstellung eines leeren Datensatzes als Vorlage        
                adj_diff_icc <- data.frame("IccAdjDifference_1"= rep(NA,n_row))
                adj_diff_icc_col_names <- paste("IccAdjDifference",1:n_icc_calc,sep="_")
                adj_diff_icc[,adj_diff_icc_col_names] <- rep(NA,n_row)
  
                share_acquired_col <- maa_table[share_acquired]   
                   
                for(i in 1:n_icc_calc){
                   ## Auswahl der Spalten
                   adj_diff_icc_col <- paste("IccAdjDifference",i,sep="_")
                   icc_diff_col <- maa_table[paste(icc_diff,i,sep="_")]   
                   target_mv_prae_col <- maa_table[paste(target_mv_prae,i,sep="_-")] 
                   acquiror_mv_prae_col <- maa_table[ paste(acquiror_mv_prae,i,sep="_-")]
                   
                   
                   ## Berechnung adjustierten Differenz
                   adj_diff_icc[,adj_diff_icc_col] <- (icc_diff_col * acquiror_mv_prae_col) / (target_mv_prae_col * share_acquired_col)
                }   
                   ##names( adj_diff_icc)<-"IccAdjDifference"
            ## Anhängen der neuen Spalten an MaATable          
            maa_table <- cbind(maa_table,adj_diff_icc)
...
Icc_Period_Get <- function(DateAnnounced,DateEffective){
        ...
        ##DatPost <- DatPrae <- rep(Datum,icc_period_data$Far - icc_period_data$Close+1)
        IccPraeDat$mday <- IccPraeDat$mday - 7    
        IccPraeDat$mon <- IccPraeDat$mon - 1   
        IccPostDat$year <- IccPostDat$year + 1 
        IccPostDat$mon <- 4   
