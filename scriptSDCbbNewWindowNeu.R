...
ICCDiff <- function(SummarySdc,EventWset){
			nobs <- ((EventWset$get()$Far - EventWset$get()$Close+1)-EventWset$get()$Size+1)
			zahl <- 1:nobs
			TaIccCol <- rev(paste("TaIcc",-zahl,sep="_"))
			AcIccPraeCol <- rev(paste("AcIccPrae",-zahl,sep="_"))
			AcIccPostCol <- paste("AcIccPost",zahl,sep="_")
			TaMvCol <- rev(paste("TaMv",-zahl,sep="_"))
			AcMvPraeCol <-  rev(paste("AcMvPrae",-zahl,sep="_"))
			AcMvPostCol <- paste("AcMvPost",zahl,sep="_")
			...
			colnames(temp) <-  rev(paste("WeightedIccPrae",-zahl,sep="_"))
